...
Сделаны этап 1 и этап 2

Для проверки этапа 1 необходимо запустить simple_client.py. Клиент подключен к базе данных, находящейся в файле bookkeeper/bookkeeper.db через sqlite_repository.
```commandline
poetry run python bookkeeper/simple_client.py
```
Для проверки этапа 2 необходимо запустить main_window.py. Реализован внешний вид приложения без функциональности.
```commandline
poetry run python bookkeeper/view/main_window.py
```
Также реализована модель бюджета с функцией подсчета расходов за выбранные периоды.
